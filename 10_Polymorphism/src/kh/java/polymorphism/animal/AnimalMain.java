...
//		animal.say();
	...
	public void test3() {
		...
		arr[0] = new Tiger(); //자식타입이 부모타입 배열에 들어갈수있다
		arr[1] = new Lion();
		arr[2] = new Tiger();
		
		//forEach 반복문(향상된 for문) - 증감변수가 없음
		for(Animal a : arr) {
...
//			((Lion)a).kick();
		...
		int i = 1; //forEach 반복문에 증감식 넣고 싶을때
		for(Animal a : arr) {
...
//		public void attack(Lion t){}
	...
	public void test1() {
		...
		a1.say();
//		a1.punch(); //자식객체를 부모타입 참조변수에 담으면 
		            //자식클래스에서 선언된 메소드는 사요할 수 없다.
...
//		a2.kick();
		...
		//부모타입 -> 자식타입 : 명시적 형변환할 것(Downcasting)
